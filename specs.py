...
lm.remove('_')
lm.remove('|')
cl = ['#003399','#009900','#990000','#009999','#990099','#505505','#999900','#090090']

avkeys = ['bxmz','bymz']
avkeys = ['ruxmz','ruymz','ruzmz','ekinmz','rhomz','uxmz', 'uymz', 'uzmz','oumz','jbmz','oxmz','oymz','ozmz','bxmz','bymz','jxmz','jymz']
...
wrkdir="."
simsdir=join("/scratch/project_2000403")
lpdf=False
if lpdf:
    png = '.pdf'
...
sims_list=list(pc.math.natural_sort(glob.glob('interstellar-test')))
print(sims_list)
...
for sim in [sims]:
    ...
    if sim == "H2":
        ...
        tdelay = 20.
    try:
        av = pc.read.aver(plane_list='xy',var_names=avkeys,
                          precision='f',time_range=None)
    except:
        print("waiting {:.1f} seconds for averages to be available".format(tdelay))
        time.sleep(tdelay)
        av = pc.read.aver(plane_list='xy',var_names=avkeys,
                          precision='f',time_range=None)
    print("size av.t",av.t.size)
    ekinturb = av.xy.ekinmz-0.5*(av.xy.ruxmz**2+av.xy.ruymz**2+av.xy.ruzmz**2)/av.xy.rhomz
    ...
    par.__setattr__('SNI_area_rate',SNI_area_rate_cgs*
                                  par.unit_length**3/
                                  par.unit_velocity*1e-3/
                                  par.lxyz[2])
    par_dict[sim] = par
    pwr = pc.read.power()
    pwr.t -= tstmin
    ...
    for key in keys:
        if not key=="t" and not key=="krms":
            tmp = gf(pwr.__getattribute__(key),sigma=[0.5,1.0], mode='nearest')*par.unit_energy_density
            if "kin" in key:
                tmp *= ts.rhom.mean()
            pwr.__setattr__(key+"_smoothed", tmp)
            tmp = gf(pwr.__getattribute__(key),sigma=[2.5,1.0], mode='nearest')*par.unit_energy_density
            if "kin" in key:
                tmp *= ts.rhom.mean()
...
istart=70
for isim in [0,]:
    fig, ax = plt.subplots(1,2,sharey=False,figsize=[figsize[0]*1.7,figsize[1]*0.9])
    ilst = np.mod(np.array(iset),3)
    for sim, lsty, lw, mk in zip([sims],lst[ilst],lws[ilst],['+','<','d']):
        par = par_dict[sim]
        pwr = pwr_dict[sim]
        krms = pwr.krms[1:]/par.lxyz[0]*2*np.pi
        #it = np.where(pwr.t >= tsteps[sim][1])[0][0]
        it = 0
        label=r"${}\Omega_{{\rm Sn}},\,{:.1f}\eta_0$".format(sim[1],par.eta/eta_0)
        ax[0].loglog(krms, pwr.mag_smoothed[it,1:],
                     ls=lsty[0],color=lsty[1],lw=lw[0],
                     marker=None,markersize=3.5,fillstyle='none',
                     alpha=lw[1],label=label)
        label=r"${}\Omega_{{\rm Sn}},\,{:.0f}\nu_0$".format(sim[1],par.nu/nu_0)
        ax[1].loglog(krms, pwr.kin_smoothed[it,1:],
                     ls=lsty[0],color=lsty[1],lw=lw[0],
                     marker=None,markersize=3.5,fillstyle='none',
                     alpha=lw[1],label=label)
    ...
    plt.tight_layout()
    if not "eta" in sim:
        plt.savefig(join(figdir,'power-{}SSD'.format(sim[:2])+png))
    else:
        plt.savefig(join(figdir,'power-{}eta'.format(sim[:2])+png))
    plt.close()
exit()
for isim in [[0,1,2],[3,4,5],[6,7,8],[6,9,10]]:
    fig, ax = plt.subplots(1,2,sharey=False,figsize=[figsize[0]*1.7,figsize[1]*0.9])
    ilst = np.mod(0,3)
    for sim, lsty, lw, mk in zip(sims[isim],lst[ilst],lws[ilst],['+','<','d']):
        print(sim)
        par = par_dict[sim]
        pwr = pwr_dict[sim]
        krms = pwr.krms[1:]/par.lxyz[0]*2*np.pi
        try:
            ...
            it = -1
        label=r"${}\Omega_{{\rm Sn}},\,{:.1f}\eta_0$".format(sim[1],par.eta/eta_0)
        ax[0].loglog(krms, pwr.mag_25smoothed[it,1:],
                     ls=lsty[0],color=lsty[1],lw=lw[0],
                     marker=None,markersize=3.5,fillstyle='none',
                     alpha=lw[1],label=label)
        label=r"${}\Omega_{{\rm Sn}},\,{:.0f}\nu_0$".format(sim[1],par.nu/nu_0)
        ax[1].loglog(krms, pwr.kin_25smoothed[it,1:],
                     ls=lsty[0],color=lsty[1],lw=lw[0],
                     marker=None,markersize=3.5,fillstyle='none',
                     alpha=lw[1],label=label)
    ax[0].set_ylabel(r'$P_B(k)$ [erg cm$^{-3}$]')
    ...
    plt.tight_layout()
    if not "eta" in sim:
        plt.savefig(join(figdir,'power-{}trans'.format(sim[:2])+png))
    else:
        plt.savefig(join(figdir,'power-{}teta'.format(sim[:2])+png))
    plt.close()
...
for isim in [[9,0,7],[10,0,8]]:
    ...
    ax[1].legend(framealpha=0.4,ncol=1)
    plt.tight_layout()
    plt.savefig(join(figdir,'power-{:.1f}etapair'.format(par.eta/eta_0)+png))
    plt.close()
...
for isim in [[0,3,6],[1,4,7],[2,5,8]]:
    ...
    plt.tight_layout()
    if not "eta" in sim:
        plt.savefig(join(figdir,'power-{}trans'.format(par.eta)+png))
    else:
        plt.savefig(join(figdir,'power-{}teta'.format(par.eta)+png))
    plt.close()
...
for isim in [[0,3,6],]:
    ...
    ax[1].legend(framealpha=0.4,ncol=1)
    plt.tight_layout()
    plt.savefig(join(figdir,'power-{}fast'.format(par.eta)+png))
    plt.close()
    ...
    for sim, lsty, lw, mk in zip(sims[isim],lst[ilst],lws[ilst],['+','<','d']):
        if not sim==sims[0]:
            ...
            try:
                ...
                it = -1
            label=r"${}\Omega_{{\rm Sn}},\,{:.1f}\eta_0,\,{:.0f}pc$".format(sim[1],par.eta/eta_0,gd.dx*1e3)
            ax[0].loglog(krms, pwr.mag_25smoothed[it,1:],
                         ls=lsty[0],color=lsty[1],lw=lw[0],
                         marker=None,markersize=3.5,fillstyle='none',
                         alpha=lw[1],label=label)
            label=r"${}\Omega_{{\rm Sn}},\,{:.0f}\nu_0,\,{:.0f}pc$".format(sim[1],par.nu/nu_0,gd.dx*1e3)
            ax[1].loglog(krms, pwr.kin_25smoothed[it,1:],
                         ls=lsty[0],color=lsty[1],lw=lw[0],
                         marker=None,markersize=3.5,fillstyle='none',
                         alpha=lw[1],label=label)
...
for isim in [[12,0,11]]:
    ...
    for sim, lsty, lw, mk in zip(sims[isim],lst[ilst],lws[ilst],['+','<','d']):
        if not sim==sims[0]:
            ...
            if sim == "H2":
                ...
                tshift=0
                it = np.where(pwr.t >= tsteps[sim][1])[0][0]
            label=r"${}\Omega_{{\rm Sn}},\,{:.0f}\nu_0,\,{:.1f}\eta_0,\,{:.0f}pc$".format(sim[1],par.nu/nu_0,par.eta/eta_0,gd.dx*1e3)
            ax.plot(pwr.t[it:]+tshift, pwr.mag[it:,:3].sum(axis=1)/pwr.mag[it:].sum(axis=1),
                         ls=lsty[0],color=lsty[1],lw=lw[0],
                         marker=None,markersize=3.5,fillstyle='none',
                         alpha=lw[1],label=label)
    ...
    plt.tight_layout()
    if not "eta" in sim:
        plt.savefig(join(figdir,'tpower-{}'.format(sim[:2])+png))
    else:
        plt.savefig(join(figdir,'tpower-{}'.format(sim[:2])+png))
    plt.close()
...
for isim in [[12,0,11]]:
    ...
    plt.tight_layout()
    if not "eta" in sim:
        plt.savefig(join(figdir,'tpower-{}x'.format(sim[:2])+png))
    else:
        plt.savefig(join(figdir,'tpower-{}'.format(sim[:2])+png))
    plt.close()
    ...
    for sim, lsty, lw, mk in zip(sims[isim],lst[ilst],lws[ilst],['+','<','d']):
        if not sim==sims[0]:
            ...
            if sim == "H2":
                ...
                tshift=0
                it = np.where(pwr.t >= tsteps[sim][1])[0][0]
            label=r"${}\Omega_{{\rm Sn}},\,{:.0f}\nu_0,\,{:.1f}\eta_0$".format(sim[1],par.nu/nu_0,par.eta/eta_0)
            ax.plot(pwr.t[it:]+tshift, pwr.mag[it:,:3].sum(axis=1)/pwr.mag[it:].sum(axis=1),
                         ls=lsty[0],color=lsty[1],lw=lw[0],
                         marker=None,markersize=3.5,fillstyle='none',
                         alpha=lw[1],label=label)
...
for isim in [[0,3,6],[1,4,7],[2,5,8]]:
    ...
    for sim, lsty, lw, mk in zip(sims[isim],lst[ilst],lws[ilst],['+','<','d']):
        ...
        if sim == "H2":
            ...
            it = np.where(pwr.t >= tsteps[sim][1])[0][0]
        label=r"${}\Omega_{{\rm Sn}},\,{:.0f}\nu_0,\,{:.1f}\eta_0$".format(sim[1],par.nu/nu_0,par.eta/eta_0)
        ax[iplot].plot(pwr.t[it:]+tshift, pwr.mag[it:,:3].sum(axis=1)/pwr.mag[it:].sum(axis=1),
                     ls=lsty[0],color=lsty[1],lw=lw[0],
                     marker=None,markersize=3.5,fillstyle='none',
                     alpha=lw[1],label=label)
...
for isim in [[7,9],[8,10]]:
    ...
    ax[1].legend(framealpha=0.4,ncol=1)
    plt.tight_layout()
    plt.savefig(join(figdir,'power-{}teta{:.1f}'.format(sim[:2],par.eta/eta_0)+png))
    plt.close()
...
j = 0
for isim in [[12,11]]:
    for sim, lsty, lw, mk in zip(sims[isim],lst[[6+j,5+j]],lws[[0,4]],['+','<']):
        ts=ts_dict[sim]
        par=par_dict[sim]
        gd=gd_dict[sim]
        if sim == "H2":
        ...
        label=r"${}\Omega_{{\rm Sn}},\,{:.0f}\nu_0,\,{:.1f}\eta_0,\,{:.0f}$ pc".format(sim[1],par.nu/nu_0,par.eta/eta_0,gd.dx*1e3)
        plt.semilogy(ts.t[::skip]+tshift,ts.eB[::skip],
                     ls=lsty[0],color=lsty[1],lw=lw[0],
                     marker=None,markersize=3.5,fillstyle='none',
                     alpha=lw[1],label=label)
    j -=3
...
for isim in [[0,1,2],[3,4,5],[6,7,8]]:
    plt.figure(figsize=figsize)
    for sim, lsty, lw, mk in zip(sims[isim],lst[isim],lws[isim],['+','<','d']):
        ts=ts_dict[sim]
        par=par_dict[sim]
        skip=1#max(int(ts.t.size/40),1)
        label=r"${}\Omega_{{\rm Sn}},\,{:.0f}\nu_0,\,{:.1f}\eta_0$".format(sim[1],par.nu/nu_0,par.eta/eta_0)
        plt.semilogy(ts.t[::skip],ts.eB[::skip],
                     ls=lsty[0],color=lsty[1],lw=lw[0],
                     marker=None,markersize=3.5,fillstyle='none',
                     alpha=lw[1],label=label)
    plt.legend(fontsize=0.85*fontsize,framealpha=0.5,ncol=1)
    ...
    plt.ylabel(r"$e_B/\overline{e_K^\prime}$")
    plt.tight_layout()
    plt.savefig(figdir+'eBt{}all'.format(sim[:2])+png)
    plt.close()

for isim in [[0,3,6],[1,4,7],[2,5,8]]:
    plt.figure(figsize=figsize)
    for sim, lsty, lw, mk in zip(sims[isim],lst[isim],lws[[0,1,2]],['+','<','d']):
        ts=ts_dict[sim]
        par=par_dict[sim]
        skip=1#max(int(ts.t.size/40),1)
        label=r"${}\Omega_{{\rm Sn}},\,{:.0f}\nu_0,\,{:.1f}\eta_0$".format(sim[1],par.nu/nu_0,par.eta/eta_0)
    ...
    plt.ylabel(r"$e_B/\overline{e_K^\prime}$")
    plt.tight_layout()
    plt.savefig(figdir+'eBt{}all'.format(sim[4:])+png)
    plt.close()

for isim in [[6,9,10]]:
    plt.figure(figsize=figsize)
    for sim, lsty, lw, mk in zip(sims[isim],lst[[6,4,2]],lws[[0,4,8]],['+','<','d']):
        ts=ts_dict[sim]
        par=par_dict[sim]
...
for isim in [[9,7],[10,8]]:
    for sim, lsty, lw, mk in zip(sims[isim],lst[[6+j,6+j]],lws[[0,4]],['+','<']):
        ts=ts_dict[sim]
        par=par_dict[sim]
        print("sim, nu, eta",sim, par.nu, par.eta)
        skip=1#max(int(ts.t.size/40),1)
